...
class LogSumExp(BaseProblem):
    ...
    def gradient(self, x: np.ndarray) -> np.ndarray:
        clp = 150
        w = self.A @ x - self.b                     # (dims,)
        z = clp * np.tanh(w / clp)                   # clipped (dims,)
        sech2 = 1.0 - np.tanh(w / clp) ** 2          # (dims,)
        # Numerically stable softmax of z
        z_shift = z - np.max(z)
        exp_z = np.exp(z_shift)
        p = exp_z / exp_z.sum()                       # softmax (dims,)
        return self.A.T @ (sech2 * p) + self.lmbd * x
